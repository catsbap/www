	...
	<h1>Pending Approval</h1>
		<table border="0px solid">
			...
			foreach($timesheet_dates as $timesheet) {
			print_r($person);				
				//list($timesheet_dates) = $timesheet->getTimesheetDatesByTimesheetId($timesheet->getValueEncoded("timesheet_id"));
				?>
				<tr><td style="background-color:grey;" colspan=8><?php echo $timesheet->timesheet_start_date; ?> THROUGH <?php echo $timesheet->timesheet_end_date; ?></td></tr>
				<tr><td colspan=8>
				
				<?php 
				//$person = Person::getPersonById($timesheet_dates->getValueEncoded("person_id"));
				?><a href="timesheet_submitted.php?timesheet_id=<?php echo $timesheet->timesheet_id;?>"><?php echo $person[0]->person_first_name; echo(" "); echo $person[0]->person_last_name;?> </a><b><?php //echo $timesheet_item->sumTimesheetHours($timesheet->getValueEncoded("timesheet_id"))["sum(timesheet_hours)"]?>Hours</b></td></tr> 
				
		<?php 
				//$timesheets_for_approval[] = $timesheet->timesheet_id;
				//if (isset($_GET["timesheet_id"]) && $_GET["timesheet_id"] == $timesheet->timesheet_id) {
